...
class CustomQuery {
  ...
  Future<List<Units>> getUnitByCommunityId(int communityId, context) async {
    ...
    for (int i = 0; i < propertyList.length; i++) {
      if (communityId == propertyList[i].communityId)
        resultList.add(propertyList[i].id);
    }

    QuerySnapshot data =
        await unitRef.where('propertyId', whereIn: resultList).get();
    List unitList = data.docs.map((e) => e.data()).toList();
    // ignore: avoid_print
    print(unitList);
    List<Units> units = unitList
  ...
  Future<List<Units>> getPropertyStatus(List<Status> status) async {
    QuerySnapshot data = await unitRef
        .where('statusId',
            whereIn: List.generate(status.length, (index) => status[index].id))
        .get();
    List unitList = data.docs.map((e) => e.data()).toList();
    List<Units> units = unitList
        .map((e) => Units(
  ...
  Future<List<Units>> getByPropertyType(
      List<PropertyType> propertyType, context) async {
    ...
    QuerySnapshot data =
        await unitRef.where('propertyId', whereIn: propertyId).get();
    List unitList = data.docs.map((e) => e.data()).toList();
    List<Units> units = unitList
        .map((e) => Units(
              status: e['status'],
  ...
  Future<List<Units>> getComplexQuery(
    ...
    List<int> resultList = [];
    for (int i = 0; i < propertyList.length; i++) {
      if (propertyTypeId.isNotEmpty &&
          !propertyTypeId.contains(propertyList[i].propertyTypeId)) {
        continue;
      }
      if (communityId.isNotEmpty &&
          !communityId.contains(propertyList[i].communityId)) continue;
      if (propertyInt.isNotEmpty && !propertyInt.contains(propertyList[i].id))
        continue;
      resultList.add(propertyList[i].id);
    }
    // ignore: avoid_print
    ...
    if (resultList.isNotEmpty) {
      ...
      if (statusId.isNotEmpty) {
        units = units
            .where((element) => statusId.contains(element.statusId))
            .toList();
      }

      if (bedRoomList.isNotEmpty) {
        units = units
            .where((element) => bedRoomList.contains(element.bedrooms))
            .toList();
      }
      return units;
      // ignore: curly_braces_in_flow_control_structures
    } else if (status.isNotEmpty)
      query = query.where('status', whereIn: statusId);
