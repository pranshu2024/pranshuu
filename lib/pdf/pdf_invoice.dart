...
class PdfInvoice {
  static Future<File> generate(
      {required Properties properties, required Units units}) async {
    // print(data[0]);
    final pdf = Document();
    final logo = pw.MemoryImage(
      (await rootBundle.load('assets/images/logo_white_background.jpg'))
          .buffer
          .asUint8List(),
    );
    ...
    for (int i = 0; i < properties.installment.length; i++) {
      ...
      DateTime newDate =
          date.add(Duration(days: properties.installment[i]['daysAfter']));
    ...
    pdf.addPage(MultiPage(
      build: (context) => [
        ...
        pw.Padding(
          padding:
              const pw.EdgeInsets.symmetric(horizontal: PdfPageFormat.cm * 1),
          child: pw.Column(children: [
            ...
            pw.SizedBox(height: PdfPageFormat.cm * 1),
            buildInvoice(
                ['Milestone', 'Percentage', 'Date', 'Amount\n(AED)'], list),
          ]),
        ),
  ...
  static Widget buildTitle(
      ImageProvider image, Properties property, pw.ImageProvider logo) {
    return Row(children: [
      pw.Image(image,
  ...
  static Widget buildInvoice(List<String> head, List<List> data) {
    ...
    return Table.fromTextArray(
      ...
      cellStyle: pw.TextStyle(
          fontSize: (headers.length > 7) ? 7 : 11, color: PdfColors.black),
      cellAlignments: {
  ...
  static Widget buildPropertyDetails(Properties properties, Units units) {
    return pw.Column(children: [
      ...
      buildText(
          title: 'Community',
          value: communityMap[properties.communityId]!.name),
      buildText(title: 'Unit Name', value: units.name),
      buildText(title: 'Unit Position', value: units.position),
      buildText(
          title: 'Property Type',
          value: propertyTypeMap[properties.propertyTypeId]!.name),
